...
const Header = () => {
  ...
  const ctaIsAnchor = header.ctaLink.startsWith('#');
  const CtaLink = ({ className, children }) =>
    ctaIsAnchor ? (
      <a href={header.ctaLink} onClick={(e) => scrollToSection(e, header.ctaLink)} className={className}>
        {children}
      </a>
    ) : (
      <a href={header.ctaLink} target="_blank" rel="noopener noreferrer" className={className}>
        {children}
      </a>
    );

  return (
    <header className={cn('sticky top-0 z-50 w-full transition-all duration-300', isScrolled ? 'border-b border-border/40 bg-background shadow-md' : 'bg-background')}>
      <div className="container mx-auto flex h-20 items-center justify-between px-4">
        <Link to="/" onClick={(e) => scrollToSection(e, '#hero')} className="flex items-center space-x-2">
          <img src={brand.logo} alt={brand.name} className="w-[160px] transition-[filter] duration-300 dark:brightness-0 dark:invert" />
        </Link>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button asChild className="hidden lg:inline-flex cta-button-accent rounded-full px-6 py-3">
            <CtaLink>
              <WhatsAppIcon className="mr-2 h-4 w-4" /> {header.ctaText}
            </CtaLink>
          </Button>
          <Button variant="ghost" size="icon" className="md:hidden text-muted-foreground hover:text-primary" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
      ...
      {mobileMenuOpen && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden border-t border-border/40 bg-background/95 shadow-lg">
          <nav className="flex flex-col space-y-2 p-4">
            {header.navLinks.map((link) => (
              <a key={link.href + link.text} href={link.href} onClick={(e) => handleNavClick(e, link.href)} className="block rounded-md px-3 py-2 text-base font-medium text-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
                {link.text}
              </a>
            ))}
